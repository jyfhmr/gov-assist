...
const PaymentForm: React.FC<PaymentFormProps> = ({ onTotalAmountChange }) => {
    ...
    const [selectedExpedite, setSelectedExpedite] = useState('72-hours');
    ...
    }, [selectedExpedite, refusalGuarantee, onTotalAmountChange, form, t]);
        ...
        <>
            ...
            <Row gutter={[32, 24]}>
                <Col xs={24} lg={16}>
                    ...
                    <Row gutter={16}>
                        <Col xs={24} sm={12}>
                            <Form.Item name="fullName" label={t('payment_form_full_name_label')} rules={[{ required: true, message: t('validation_required') }]}><Input placeholder={t('payment_form_full_name_placeholder')} /></Form.Item>
                        </Col>
                        <Col xs={24} sm={12}>
                            <Form.Item name="billingEmail" label={t('payment_form_billing_email_label')} initialValue={form.getFieldValue('email')} rules={[{ required: true, type: 'email', message: t('validation_email_invalid') }]}><Input placeholder={t('payment_form_billing_email_placeholder')} /></Form.Item>
                        </Col>
                    </Row>
                ...
                <Col xs={24} lg={8}>
                    <Card>
                        ...
                        <Form.Item name="termsAgree" valuePropName="checked" rules={[{ validator: (_, value) => value ? Promise.resolve() : Promise.reject(new Error(t('validation_must_agree'))) }]}>
                            <Checkbox>{t('payment_form_agree_terms_checkbox', { 0: <Link href="#">{t('footer_terms_service')}</Link> })}</Checkbox>
                        </Form.Item>
                    </Card>
                </Col>
            </Row>
