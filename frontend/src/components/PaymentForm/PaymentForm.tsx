...
// --- El componente ahora recibe una prop para comunicar el total al padre ---
...
const PaymentForm: React.FC<PaymentFormProps> = ({ onTotalAmountChange }) => {
    ...
    // --- Estado para mostrar el total en el resumen ---
    const [total, setTotal] = useState(0);
    const [expediteCost, setExpediteCost] = useState(0);

    // --- Efecto para calcular y comunicar el total cada vez que una opción cambia ---
    useEffect(() => {
        ...
        onTotalAmountChange(calculatedTotal); // Informa al componente padre

    }, [selectedExpedite, refusalGuarantee, onTotalAmountChange]);
            ...
            <Col xs={24} lg={16}>
                ...
                <Card bordered={false} style={{backgroundColor: '#f9f9f9'}}>
                    ...
                    <Paragraph type="secondary" style={{ marginTop: 8, marginBottom: 0 }}>
                        Includes a detailed review and submission of your application, support for up to two-way calls with the U.S. CBP if needed.
                    </Paragraph>
                </Card>
                ...
                <Checkbox checked={refusalGuarantee} onChange={(e) => setRefusalGuarantee(e.target.checked)}>
                    ...
                    <Paragraph type="secondary">Apply with confidence! Refusal Protect Guarantee safeguards your investment by refunding the processing service fee on denials.</Paragraph>
                </Checkbox>
                ...
                <Row gutter={16}>
                    ...
                    <Col xs={24} sm={12}>
                        <Form.Item name="billingEmail" label="Email" initialValue={form.getFieldValue('email')} rules={[{ required: true, type: 'email' }]}>
                ...
                {/* ... Puedes añadir más campos de dirección aquí ... */}
            ...
            <Col xs={24} lg={8}>
                <Card>
                    ...
                    <Paragraph style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <Text>U.S. ESTA Processing Service</Text> <Text>{BASE_SERVICE_COST.toFixed(2)} USD</Text>
                    </Paragraph>
                    {refusalGuarantee && (
                         <Paragraph style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <Text>Refusal Protect Guarantee</Text> <Text>{REFUSAL_GUARANTEE_COST.toFixed(2)} USD</Text>
                        </Paragraph>
                    )}
                    {expediteCost > 0 && (
                        <Paragraph style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <Text>{EXPEDITE_OPTIONS.find(opt => opt.value === selectedExpedite)?.label}</Text> <Text>{expediteCost.toFixed(2)} USD</Text>
                        </Paragraph>
                    )}
                     <Paragraph style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <Text>Government fee</Text> <Text>{GOVERNMENT_FEE.toFixed(2)} USD</Text>
                    </Paragraph>
                    <Divider />
                    <Paragraph style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <Text strong>Order Total:</Text> <Text strong>{total.toFixed(2)} USD</Text>
                    </Paragraph>
                    <Form.Item name="termsAgree" valuePropName="checked" rules={[{ validator: (_, value) => value ? Promise.resolve() : Promise.reject(new Error('You must agree to the terms')) }]}>
                        <Checkbox>I agree to the <a href="#">Terms of Service</a></Checkbox>
                    </Form.Item>
                </Card>
            </Col>
        </Row>
