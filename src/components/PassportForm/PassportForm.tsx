...
import countries from '../../data/countries.json'; // Reutilizamos nuestra lista de países
            ...
            <>
              {fields.map(({ key, name, ...restField }) => (
                <Space key={key} style={{ display: 'flex', marginBottom: 8, alignItems: 'start' }} align="baseline">
                  <Form.Item {...restField} name={[name, 'issuingCountry']} rules={[{ required: true }]} style={{width: 250}}><Select placeholder="Issuing Country">{countries.map(c => <Option key={c} value={c}>{c}</Option>)}</Select></Form.Item>
            ...
            <>
              {fields.map(({ key, name, ...restField }) => (
                <Space key={key} style={{ display: 'flex', marginBottom: 8 }} align="baseline">
                  <Form.Item {...restField} name={[name, 'country']} rules={[{ required: true }]} style={{width: 300}}><Select placeholder="Country">{countries.map(c => <Option key={c} value={c}>{c}</Option>)}</Select></Form.Item>
                  ...
                  <Row gutter={16}>
                    <Col span={24}>
                      <Form.Item {...restField} name={[name, 'country']} label="COUNTRY OF CITIZENSHIP / NATIONALITY *" rules={[{ required: true }]}>
                        <Select placeholder="Select Country" showSearch>{countries.map(c => <Option key={c} value={c}>{c}</Option>)}</Select>
                      </Form.Item>
                    </Col>
                  </Row>
                  <Row gutter={16}>
